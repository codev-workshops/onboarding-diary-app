    ...
    with open(file_path, "w", newline="", encoding="utf-8") as f:
        ...
        if "feedback" in data:
            ...
            for fb in data["feedback"]:
                writer.writerow(["", str(fb.date), fb.subject, fb.feedback_type, fb.details])
            writer.writerow([])
...
async def list_reports(
    ...
    q = select(Report).where(
        (Report.generated_by == current_user.id) | (Report.target_user_id == current_user.id)
    )
    ...
    result = await db.execute(
        q.order_by(Report.created_at.desc()).offset((page - 1) * per_page).limit(per_page)
    )
